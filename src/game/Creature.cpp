...
void Creature::LoadBotMenu(Player *pPlayer)
{
    ...
    QueryResult *result = CharacterDatabase.PQuery("SELECT guid, name, class FROM characters WHERE account='%d'",accountId);
    do
    {
        Field *fields = result->Fetch();
        uint64 guidlo = fields[0].GetUInt64();
		std::string name = fields[1].GetString();
        uint8 botclass = fields[2].GetUInt8();
		std::string word = "";
		...
		{
			if(pPlayer->GetSession()->GetHealbot(guidlo) == NULL && botclass == CLASS_PRIEST) // add (if not already in game)
			{
				word += "Recruit character '";
				word += name;
				word += "' as a healbot?";
				pPlayer->PlayerTalkClass->GetGossipMenu().AddMenuItem((uint8)9, word, guidlo, guidlo, word, false);
			}
			else if(pPlayer->GetSession()->GetHealbot(guidlo) != NULL) // remove (if in game)
			{
				word += "Dismiss '";
				word += name;
				word += "' from duty?";
